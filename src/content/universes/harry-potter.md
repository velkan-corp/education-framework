---
label: Harry Potter
kind: fictional-universe
summary: "A scalable ethics and institution-reading curriculum disguised as irresistible school fiction."
whyItBelongs: "Harry Potter works because the reader ages with the material. It is one of the best early vehicles for loyalty, prejudice, authority, sacrifice, and ambiguity that remains socially legible."
tags: [books, english]
languages: [english]
targets: [self-awareness, communication-mastery, relational-intelligence, compassion-kindness]
models: [cause-effect, incentives, emergence, model-failure]
intensity: medium
riskFlags: [violence]
socialValue: broad
substitutes: [tolkien, avatar, doctor-who]
phases:
  - phaseId: age-8-10
    tier: foundational
    title: "Harry Potter (complete series)"
    description: "Seven books that scale ethical complexity per volume — from simple good/evil to Snape's ambiguity, Dumbledore's flaws, Hallows vs Horcruxes. The most effective fiction curriculum vehicle for this age."
    mode: dim
    goals:
      - "Let ethical complexity scale with the reader's age and stamina."
      - "Use houses, friendships, and institutions to discuss loyalty, status, and authority."
    cautions: "Prioritize the books. The films compress too much moral and institutional texture to carry the same load on their own."
---
