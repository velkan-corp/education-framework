---
label: "Charlie Chaplin"
kind: creator-canon
summary: "Comedia muda que convierte la actuacion fisica en critica social y resiliencia humana."
whyItBelongs: "Chaplin ensena que la narrativa visual puede vehicular filosofia, critica de clase y precision emocional sin dialogo."
tags: [cinema, english]
languages: [english]
targets: [resilient-self-efficacy, communication-mastery, compassion-kindness, adaptability]
models: [cause-effect, inversion, complementary-opposites, via-negativa]
intensity: medium
socialValue: broad
substitutes: [pixar, calvin-hobbes, little-prince]
phases:
  - phaseId: age-8-10
    tier: recommended
    title: "El chico, Tiempos modernos, La quimera del oro"
    description: "El cine mudo ensena narrativa visual sin dialogo. Comentario social, comedia fisica como forma de arte. Una puerta de entrada para comprender que el cine no necesita palabras. Cantinflas: el Chaplin mexicano. Comentario de clase a través del slapstick en español — el espejo bilingüe de la tradición anglófona de Chaplin. Esencial para la alfabetización cultural en lengua española."
    mode: absorb
    goals:
      - "Entrenar la atencion al gesto, el ritmo y la composicion visual."
      - "Mostrar que la comedia puede vehicular critica de clase y ternura a la vez."
    projects:
      - "Crear el storyboard de un gag mudo que comunique un problema y una inversion sin dialogo."
---
